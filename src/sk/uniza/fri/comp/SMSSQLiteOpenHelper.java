...
public class SMSSQLiteOpenHelper extends SQLiteOpenHelper {
	...
	public static final String K_SMS_SENT_WAS_SENT = "sms_was_sent";

	private static final String[] DATABASE_CREATE = new String[] {
			"create table " + T_SMS_RECEIVED + "(" + K_SMS_RECEIVED_ID
					+ " integer primary key, " + K_SMS_RECEIVED_TEL
					+ " varchar(16) not null, " + K_SMS_RECEIVED_TIME
					+ " datetime not null, " + K_SMS_RECEIVED_TEXT
					+ " text not null);",
			"create table " + T_SMS_SENT + "(" + K_SMS_SENT_ID
					+ " integer primary key autoincrement, "
					+ K_SMS_SENT_RECEIVED_ID + " integer not null, "
					+ K_SMS_SENT_TIME + " datetime not null, "
					+ K_SMS_SENT_TEXT + " text not null," + K_SMS_SENT_WAS_SENT
					+ " integer null, " + " foreign key ("
					+ K_SMS_SENT_RECEIVED_ID + ") references " + T_SMS_RECEIVED
					+ "(" + K_SMS_RECEIVED_ID + "));", };
